...
Today, I encountered an interesting Xworm sample. It started with an email containing two ZIP attachments: one with a PDF and the other with an LNK file. The PDF did not seem to contain anything malicious; it was almost like they forgot to insert the link. However, the LNK file took a few twists before finally delivering the Xworm payload. Below is a high-level overview of the steps I took to analyze the file.
